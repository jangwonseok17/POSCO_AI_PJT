...
namespace FrostweepGames.Plugins.GoogleCloud.SpeechRecognition.Examples
{
	public class GCSR_Example : MonoBehaviour
	{
		...
		{
			_startRecordButton.interactable = false; // 비활성화
			_stopRecordButton.interactable = true; // 활성화
			_detectThresholdButton.interactable = false;
			_resultText.text = string.Empty; // 초기화

			_speechRecognition.StartRecord(_voiceDetectionToggle.isOn);
		}

		private void StopRecordButtonOnClickHandler() // stop 눌렀을 때
		{
			_stopRecordButton.interactable = false;
			_startRecordButton.interactable = true;
		...
		private void EndTalkigEventHandler(AudioClip clip, float[] raw) // End talking -> Finish record
		{
		...
		private void FinishedRecordEventHandler(AudioClip clip, float[] raw) // Finish record 하면서 안에 다른 함수들 쓴다!
		{
			...
			RecognitionConfig config = RecognitionConfig.GetDefault(); // 언어 language코드 바꾸고
			...
			GeneralRecognitionRequest recognitionRequest = new GeneralRecognitionRequest()
			{
				...
				{
					content = raw.ToBase64() /// ToBase64String: 바이너리 데이터를 Base64로 변환
				},
			...
			if (_longRunningRecognizeToggle.isOn) // long recognize toggle 이 true이면 LongRunningRecognize를 쓴다
			{
		...
		private void RecognizeSuccessEventHandler(RecognitionResponse recognitionResponse)
        {
			...
			string val = ""; 
			string path = @"\Quest 2\sdcard\DCIM\test.txt";
			// string path = @"/home/piai/text_save/test"+"_" + DateTime.Now.ToString("yyyy-MM-dd-HH:mm") + ".txt";

			try
			{
				val = recognitionResponse.results[0].alternatives[0].transcript; /// transcript가 받는 텍스트
      	...
      	{  
        	...
        	client.Connect(new IPEndPoint(IPAddress.Parse("141.223.140.48"), 5556));
